...
let oldRenderForms = `  filtered.forEach(f => {
...
oldRenderForms = oldRenderForms.replace(/\\\\`/g, '`');

let newRenderForms = `  filtered.forEach(f => {
    let statusClass = 'tracker-item--pending';
...
newRenderForms = newRenderForms.replace(/\\\\`/g, '`');
...
if (renderDefIdx > -1 && !code.includes('setFormFilter = function')) {
...
console.log('Forms tracker patched!');
