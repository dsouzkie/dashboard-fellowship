...
global.document = {
  getElementById: (id) => {
    return {
      value: id === 'loginPassword' ? 'under25christy' : '',
...
function calculateHealthScore(fellow) {
  ...
  if (fellow.firstReelPosted === 'Yes') score += 10;
  ...
  if (fellow.whatsappGroupAdded === 'Yes') score += 10;
  if (fellow.mtf && fellow.mtf !== '' && fellow.mtf !== 'N/A') score += 10;
  // Penalty for strikes
...
function renderDashboard() {
  ...
  const myAtRisk = myFellows.filter(f => f.fellowStatus === 'Ghosted' || f.fellowStatus === 'On Hold' || (f._autoStrikes && f._autoStrikes.length >= 2));
  ...
  const milestones = [
    ...
    { label: 'First Reel', field: 'firstReelPosted' },
  ...
  if (AppState.myFellowsViewMode === 'sheet') {
    const tableRows = myFellows.map(f => {
      ...
      return `
        <tr>
          ...
          <td><a href="#" class="fellow-name-link" data-id="${f.id}" style="color:#A78BFA; text-decoration:none; font-weight:600; cursor:pointer;">${escapeHTML(f.fellowName)} ${renderStrikeDots(f.id)}</a> ${f.intakeStatus ? `<span class="intake-badge--${f.intakeStatus === 'August Intake' ? 'august' : 'existing'}" style="margin-left:8px">${f.intakeStatus === 'August Intake' ? 'Aug Intake' : 'Existing'}</span>` : ""}</td>
    ...
    const cardsHtml = myFellows.map(f => {
      ...
      const photoHtml = photoUrl 
        ? `<img src="${photoUrl}" referrerpolicy="no-referrer" style="width: 100%; height: 160px; object-fit: cover; border-bottom: 3px solid ${TEAM_COLORS[f.team]?.primary || 'rgba(148,163,184,0.1)'};" onerror="this.src=''; this.outerHTML='<div style=\\'width:100%; height:160px; display:flex; align-items:center; justify-content:center; background:#1E293B; font-size:40px; color:#94A3B8; border-bottom: 3px solid ${TEAM_COLORS[f.team]?.primary || 'rgba(148,163,184,0.1)'};\\'>${escapeHTML(displayName.charAt(0))}</div>'" />`
...
function renderAllFellows() {
  ...
  if (AppState.allFellowsViewMode === 'sheet') {
    ...
    const cardsHtml = filtered.map(f => {
      ...
      const poc = TEAM.find(t => t.name === f.pocAssigned) || TEAM[0];
      ...
      <header class="page-header">
        ...
        <div class="flex" style="gap: 10px;">
          ...
          ${AppState.currentUser.isAdmin ? `
            <button class="btn btn--secondary" onclick="syncFromSheets()">🔄 Refresh Data</button>
...
function endStrikePhase() {
  ...
  POCS.forEach(poc => {
    const revs = AppState.strikeReviews[phaseId] && AppState.strikeReviews[phaseId][poc.name];
    if (revs) {
...
function renderFellowProfile(fellowId) {
  try {
    ...
    if (acceptance && acceptance.photo) {
      ...
      // photoUrl = getDriveImageUrl(alumni.nominatedFellowPhoto); // User requested only FAF photo
    }
...
function bindEvents() {
  ...
  document.querySelectorAll('select.form-select[data-filter-type]').forEach(select => {
    select.addEventListener('change', (e) => {
      ...
      else if (type === 'launched') AppState.filterLaunched = value;
      render();
    });
  });
...
setTimeout(() => {
  global.AppState.selectedTeamUser = global.TEAM[0]; // Christy
  try {
    login();
