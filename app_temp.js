...
const FIELD_KEYS = [
  ...
  'followersCount', 'fellowStatus', 'clubMade', 'clubPageLaunched', 'reelsPostedWeek1', 'reelsIn7Days7Posts', 'reelsPostedWeek2', 'contentPiecesPosted', 'comments',
...
function render() {
  ...
  if (AppState.fellows) {
    let changed = false;
    AppState.fellows.forEach(f => {
      ['firstReelPosted', 'whatsappGroupAdded', 'mtf', 'clubRecruitmentCampaign'].forEach(k => {
        if (k in f) {
          delete f[k];
          changed = true;
        }
  ...
  for (let i = 0; i < count; i++) {
    gridHtml += `
      <tr class="mass-add-row">
        ...
        <td>
          <input type="text" list="dl-poc" class="form-input ma-poc" placeholder="POC">
        </td>
        <td>
          <input type="text" list="dl-status" class="form-input ma-status" placeholder="Status" value="Active">
        </td>
        <td>
